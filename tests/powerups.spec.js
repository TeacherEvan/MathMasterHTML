...
test.describe('Power-Up Two-Click System', () => {
    ...
    test.beforeEach(async ({ page }) => {
        ...
        await page.waitForSelector('#solution-display', { timeout: 10000 });
    ...
    test('should deselect power-up when clicking same one again', async ({ page }) => {
        // Select spider
        await page.click('[data-testid="powerup-spider"]');
    ...
    test('should place devil power-up on second click', async ({ page }) => {
        ...
        await page.click('#solution-display');
    ...
    test('should not allow selection when inventory is 0', async ({ page }) => {
        ...
        // Verify not selected
        const isSelected = await page.evaluate(() => {
            return window.wormSystem?.powerUpSystem?.selectedPowerUp === 'chainLightning';
        });
    ...
    test('should switch selection when clicking different power-up', async ({ page }) => {
        // Select spider
        await page.click('[data-testid="powerup-spider"]');
...
test.describe('Power-Up Chain Lightning', () => {
    ...
    test('should refund chain lightning if no worms to target', async ({ page }) => {
        // Get initial count
        const initialCount = await page.evaluate(() => {
...
test.describe('Game Flow Integration', () => {
    ...
    test('game page loads successfully', async ({ page }) => {
        ...
        await expect(page.locator('#rain-display')).toBeVisible();
    ...
    test('level select page works', async ({ page }) => {
        ...
        await expect(page.locator('.level-button, .level-card, [data-level]').first()).toBeVisible({ timeout: 5000 });
    });
