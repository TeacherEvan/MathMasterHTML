...
class WormSystem {
    ...
    constructor() {
        ...
        this.cachedAllSymbols = null;      // Cache for ALL symbols (purple worm targeting)
    ...
    // Get ALL stealable symbols in solution container (for purple worms)
    // This queries hidden-symbol (red) AND revealed-symbol (blue)
    getCachedAllSymbols() {
        const now = Date.now();
        if (!this.cachedAllSymbols || (now - this.allSymbolsCacheTime) > this.CACHE_DURATION_TARGETS) {
            // Query ALL symbol elements that can be stolen
    ...
    _updateWormRushingToTarget(worm) {
        ...
        // PERFORMANCE: Use cached revealed symbols for regular worms
        const revealedSymbols = this.getCachedRevealedSymbols();
        ...
        // PURPLE WORM FIX: If no targetElement found, search ALL symbols (not just revealed cache)
        // Purple worms can steal both hidden (red) AND revealed (blue) symbols
        if (!targetElement && worm.isRushingToTarget) {
            // Use getAllSymbols cache which includes BOTH hidden-symbol AND revealed-symbol
            ...
            if (worm.isPurple && worm.canStealBlue) {
                ...
                if (redSymbols.length > 0) {
                    availableSymbols = redSymbols;
                    console.log(`🟣 Purple worm ${worm.id} found ${redSymbols.length} RED symbols to target`);
                } else {
                    availableSymbols = allSymbols.filter(el => el.classList.contains('revealed-symbol'));
                    console.log(`🟣 Purple worm ${worm.id} falling back to ${availableSymbols.length} BLUE symbols`);
                }
            } else {
                // Regular worms only target hidden (red) symbols
            ...
            if (availableSymbols.length > 0) {
                ...
                if (nearestSymbol) {
                    ...
                    worm.targetElement = nearestSymbol; // Also store element reference
                    targetElement = nearestSymbol;
                    console.log(`🟣 Purple worm ${worm.id} targeting nearest symbol: "${worm.targetSymbol}" (${nearestDistance.toFixed(0)}px away)`);
                }
            } else {
                console.log(`🟣 Purple worm ${worm.id} found NO available symbols to steal`);
            }
        }
